...
//                            response.getMessage());
        ...
        //add start by 李鹏 初始化引擎插件 (非必须，只有第三方插件需要支持)
        try {
    ...
    private static void initialPressureContext(EngineRunConfig config) {
        ...
        if (null == reportId || 0 == reportId) {
            ...
            logger.error("无效的reportId.[{}]", reportId);
            System.exit(-1);
        }
